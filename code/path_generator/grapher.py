...
class BGGraph(object):
	...
	def __init__(self, args):
		...
		self.ent2id          = json.load(open(os.path.join(self.data_dir, 'ent2ids')))
		self.id2ent          = id2name(self.ent2id)
		self.rel2id          = json.load(open(os.path.join(self.data_dir, 'relation2ids')))
		self.id2rel          = id2name(self.rel2id)

		self.out             = json.load(open(os.path.join(self.data_dir, 'path_subgraph')))
